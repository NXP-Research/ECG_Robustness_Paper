...
from training import evaluate, get_center, train
...
def retrain_pareto(pareto_front, numpy_data):
    for i, trial in enumerate(pareto_front):
        ...
        if len(run_aucs) == 5:
            ...
            with open(
                f"ecg_outlier_detection/deepsvdd/results_{CONDITION}.txt",
                "a",
            ) as log_file:
                log_file.write(f"\nModel {i + 1}:\n")
...
if __name__ == "__main__":
    ...
    db_name_part = f"deepsvdd_{CONDITION}"
    study = create_study(
        study_name=f"nas_{db_name_part}",
        storage=f"sqlite:///ecg_outlier_detection/deepsvdd/nas_{db_name_part}.db",
    ...
    while (
        ...
        initial_num_filter = trial.suggest_categorical(
            "initial_num_filter", [12, 16, 24, 32, 64, 128, 256, 512, 1024]
        )
        ...
        for i in range(num_layers):
            num_filters_list.append(
                trial.suggest_categorical(
                    f"filters_{i}", [12, 16, 24, 32, 64, 128, 256, 512, 1024]
                )
            )
