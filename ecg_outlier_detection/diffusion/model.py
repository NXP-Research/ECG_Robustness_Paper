...
# The following functions and classes were all adapted from denoising-diffusion-pytorch
# Source: https://github.com/lucidrains/denoising-diffusion-pytorch/blob/main/denoising_diffusion_pytorch/denoising_diffusion_pytorch_1d.py
...
class GaussianDiffusion1D(Module):
    ...
    @torch.no_grad()
    def p_sample_loop(
        self, shape, return_noise=False, model_forward_kwargs: dict = dict()
    ):
        ...
        for t in tqdm(
            ...
            img, x_start = self.p_sample(
                img, t, self_cond, model_forward_kwargs=model_forward_kwargs
            )
    ...
    @torch.no_grad()
    def p_sample_loop_from_input(
        self, x_input, timestep, model_forward_kwargs: dict = dict()
    ):
        device = self.betas.device

        img = x_input.to(device)
    ...
    @torch.no_grad()
    def sample(
        self, batch_size=16, return_noise=False, model_forward_kwargs: dict = dict()
    ):
        ...
        shape = (
            ...
            else (batch_size, seq_length, channels)
        )
        return sample_fn(
            shape, return_noise=return_noise, model_forward_kwargs=model_forward_kwargs
        )
    ...
    def forward(self, img, *args, **kwargs):
        (
            ...
            device,
            seq_length,
        ) = img.shape[0], img.shape[self.seq_index], img.device, self.seq_length
...
class Trainer1D(object):
    def __init__(
        ...
        *,
        ...
        self.eval_dl = eval_dataset
        self.test_dl = test_dataset
        ...
        self.anomaly_noise_steps = anomaly_noise_steps
    ...
    def train(self):
        ...
        patience_checks = self.patience // self.evaluate_every
        lr_status = 0
        best_val_auc = -1
        test_auc = -1
        ...
        with tqdm(
            ...
            while self.step < self.train_num_steps:
                ...
                if accelerator.is_main_process:
                    ...
                    if self.step != 0 and self.step % self.evaluate_every == 0:
        ...
        if not self.break_early:
            self.ema.ema_model.eval()
            with torch.no_grad():
                test_auc = self._evaluate(mode="test")
            print(test_auc)
        return best_val_auc, test_auc

    def _evaluate(self, mode="eval"):
        accelerator = self.accelerator
        device = accelerator.device
        dist_array = []
        label_array = []
        dl = self.eval_dl if mode == "eval" else self.test_dl
        for batch_data, batch_labels in dl:
            batch_data = batch_data.to(device)
            batch_data = self.ema.ema_model.normalize(batch_data)
            noise = torch.randn_like(batch_data)
            t = torch.full(
                (batch_data.shape[0],),
                self.anomaly_noise_steps - 1,
                device=device,
                dtype=torch.long,
            )
            x_noise = self.ema.ema_model.q_sample(x_start=batch_data, t=t, noise=noise)
            output = self.ema.ema_model.p_sample_loop_from_input(
                x_noise, timestep=self.anomaly_noise_steps
            )
            dist = torch.sum((output - batch_data) ** 2, dim=(1, 2))
            dist_array.append(dist.cpu().numpy())
            label_array.append(batch_labels.cpu().numpy())
        dist_array = np.concatenate(dist_array)
        label_array = np.concatenate(label_array)
        auc = roc_auc_score(label_array, dist_array)
        return auc
