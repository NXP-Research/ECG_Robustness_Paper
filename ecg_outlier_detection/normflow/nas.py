...
def retrain_pareto(pareto_front, numpy_data):
    for i, trial in enumerate(pareto_front):
        ...
        if len(run_aucs) == 5:
            ...
            with open(
                f"ecg_outlier_detection/normflow/results_{config.ARCHITECTURE.lower()}_{CONDITION}.txt",
                "a",
            ) as log_file:
                log_file.write(f"\nModel {i + 1}:\n")
...
if __name__ == "__main__":
    ...
    db_name_part = f"normflow_{config.ARCHITECTURE.lower()}_{CONDITION}"
    study = create_study(
        study_name=f"nas_{db_name_part}",
        storage=f"sqlite:///ecg_outlier_detection/normflow/nas_{db_name_part}.db",
            ...
            for j in range(num_layers):
                filter_list.append(
                    trial.suggest_categorical(
                        f"filter_{j}", [12, 16, 24, 32, 64, 128, 256, 512, 1024]
                    )
                )
