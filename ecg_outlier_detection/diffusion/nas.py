...
import optuna
import torch
from model import GaussianDiffusion1D, Trainer1D, Unet1D, get_model_size
...
def retrain_pareto(pareto_front, numpy_data):
    for i, trial in enumerate(pareto_front):
        ...
        if len(run_aucs) == 5:
            ...
            with open(
                f"ecg_outlier_detection/diffusion/results_{CONDITION}.txt",
                "a",
            ) as log_file:
                log_file.write(f"\nModel {i + 1}:\n")
...
if __name__ == "__main__":
    ...
    db_name_part = f"diffusion_{CONDITION}"
    study = create_study(
        study_name=f"nas_{db_name_part}",
        storage=f"sqlite:///ecg_outlier_detection/diffusion/nas_{db_name_part}.db",
