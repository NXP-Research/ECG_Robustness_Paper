...
from training import evaluate_by_method, train
...
def retrain_pareto(pareto_front, numpy_data):
    for i, trial in enumerate(pareto_front):
        ...
        if len(run_aucs) == 5:
            ...
            with open(
                f"ecg_outlier_detection/prediction/results_{CONDITION}.txt",
                "a",
            ) as log_file:
                log_file.write(f"\nModel {i + 1}:\n")
...
if __name__ == "__main__":
    ...
    study = create_study(
        study_name=f"nas_prediction_{CONDITION}",
        storage=f"sqlite:///ecg_outlier_detection/prediction/nas_prediction_{CONDITION}.db",
