...
def retrain_pareto(pareto_front, numpy_data):
    for i, trial in enumerate(pareto_front):
        ...
        if len(run_aucs) == 5:
            ...
            with open(
                f"ecg_noise_detection/autoencoder/results_{config.ARCHITECTURE.lower()}.txt",
                "a",
            ) as log_file:
                log_file.write(f"\nModel {i + 1}:\n")
...
if __name__ == "__main__":
    ...
    db_name_part = f"autoencoder_{config.ARCHITECTURE.lower()}"
    study = create_study(
        study_name=f"nas_{db_name_part}",
        storage=f"sqlite:///ecg_noise_detection/autoencoder/nas_{db_name_part}.db",
    ...
    while (
        ...
        for i in range(num_layers):
            params[f"filter_{i}"] = trial.suggest_categorical(
                f"filter_{i}", [4, 8, 16, 24, 32, 64, 128, 256, 512, 1024]
            )
